...
@implementation FMDownloadRequestHandler

+ (instancetype)sharedInstance
{
    static dispatch_once_t one;
    static FMDownloadRequestHandler *handler;
    dispatch_once(&one, ^{
        handler = [self new];
    });
    return handler;
...
{
    NSString *url = [[downloadReq downloadUrl] stringByURLEncoding];//url encoding
...
-(void)completionHandle:(NSURLResponse *)response filePath:(NSURL*)filePath error:(NSError *)error downloadReq:(FMDownloadRequest *)req
{
    FMLog(@"destinationPath:%@",req.destinationPath);
...
-(void)downloadingHandle:(NSProgress *)progress downloadReq:(FMDownloadRequest *)req
{
